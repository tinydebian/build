...
readonly APP_SOURCE_DIR=$CUR_DIR/../app.default
readonly VERSION_NUMBER=`cat $APP_SOURCE_DIR/debian_overlay/root/version.txt`
cd $APP_SOURCE_DIR
...
readonly SDCARD_IMG=$TARGET_DIR/td_${VERSION_NUMBER}_${BUILD_NUMBER}_sdcard.img
...
gen_rootfs_part() {
	...
	# copy kernel modules
	sudo cp -r $OUT_DIR/kernel/lib/modules  $TARGET_DIR/rootfs/lib
	sudo chown root:root  $TARGET_DIR/rootfs/lib/modules/

	# copy app
	sudo cp -r $OUT_DIR/app.default/debian_overlay/*  $TARGET_DIR/rootfs/
