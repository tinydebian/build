...
# Preparation:
# Run this script in Ubuntu 18.04.x
...
readonly DISTRO=buster
readonly MIRROR=${MIRROR:-http://ftp.cn.debian.org/debian/}
...
bootstrap() {
	...
	sudo debootstrap --arch=arm64 --foreign $DISTRO $TARGET_DIR $MIRROR
	...
	sudo LANG=C.UTF-8 LANGUAGE==C.UTF-8 LC_ALL=C.UTF-8 chroot $TARGET_DIR /debootstrap/debootstrap --second-stage --no-check-gpg $MIRROR
}
